...
# Some rules cover the whole fleet — only one host may run the Claude keepalive
# job, for example — so check the whole file, not only the hosts this run
# touches. just bootstrap gets the same check through doctor.sh.
"$repo_root/scripts/validate-fleet.py" "$fleet_inventory_file"
